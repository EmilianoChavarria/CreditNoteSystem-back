...
class NotificationService
{
    ...
    public function notifyForecastPendingApproval(ForecastChangeRequest $changeRequest, string $clientName = ''): void
    {
        ...
        $this->createAndBroadcast(
            ...
            message: "Se propuso un cambio de monto para el mes {$changeRequest->month}/{$changeRequest->year} — Cliente #{$changeRequest->idClient}{$client}. Monto propuesto: \${$changeRequest->proposedAmount}.",
        );
    }
    ...
    public function notifyForecastApproved(ForecastChangeRequest $changeRequest, User $approver, string $clientName = ''): void
    {
        ...
        $this->createAndBroadcast(
            ...
            message: "{$approver->fullName} aprobó el monto de \${$changeRequest->proposedAmount} para el mes {$changeRequest->month}/{$changeRequest->year} — Cliente #{$changeRequest->idClient}{$client}. El nuevo objetivo de ventas ha sido confirmado.",
        );
    }

    public function notifyForecastRejected(ForecastChangeRequest $changeRequest, User $rejector, string $clientName = ''): void
    {
        $client = $clientName ? " ({$clientName})" : '';
